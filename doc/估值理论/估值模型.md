...
> **估值对象是"可分派现金的能力"，不是资产负债表。**

本体系只对公司持续产生可分派现金的利润能力估值；**账上现金、净资产不参与估值**——现金用于维持后续发展、或管理层不分配时，不构成股东价值（除非管理层用分红/回购证明其可分派）。这一条是明示的设计选择（"盈利力/所有者收益"学派），而非遗漏。
...
**过渡期统一 3 年（风险补偿，不调整）**：过渡期**不是**针对个股调整的参数——它是对**统一折现率**的风险补偿。本模型折现率 R = rf + 5.5% 对所有公司一致、不因个股风险上调；而高增长公司的增长失速风险高于成熟龙头。三年衰减期的作用：

- **对成熟龙头**（g3 已接近个位数）：三年衰减恰好合理，风险补偿极小——对应其真实折现率本就偏低，统一 R 对它不构成低估；
- **对高增长公司**（g3 仍高）：三年衰减大概率短于实际成长窗口，用"更短的远期现金流"补偿统一折现率偏低的偏差——风险恰好被惩罚在**远期增速的不确定性**上，而非近期确定的现金流（比上调折现率更精准，后者会连带压低近期现金流）。

**统一 3 年后，估值自由度只剩利润路径 [g1, g2, g3] 与兑现系数 θ（θ₀ / θₓ 的测定），无其他个股参数可调。**
...
**说明（统一风险补偿的辩护）**：本模型不对个股调整折现率（等价于假设所有股票 β=1）——个股风险差异通过**压低利润路径**进入估值（确定性等价思想：把不确定性的代价从分母的折现率搬到分子的利润预测）。代价是忽略了系统性风险暴露差异（周期股 vs 公用事业），换来的是全市场可比性与不可操纵性——**一致性 > 精确性**。
...
**参数来源（拟合函数，无独立经济含义）**：$g_\infty$ 是**饱和型拟合函数**——终局增速随名义 GDP 增速递增但边际递减，渐近收敛至 4% 上限（m→∞ → 4%）；参数（0.35、0.04）由**历史 GDP 增速与社零增速的回归测试校准**得出，非主观选取。其作用是提供**机械一致**的终局增速映射，并保证 R − g∞ 利差始终为正（终值有界）。当 m ∈ [3, 5] 时 g∞ ∈ [2.05%, 2.55%]，落在专业常用的"成熟企业永续增速 2—3%（低于名义 GDP）"区间——稳健性来自**利差结构与全市场一致性**，不依赖参数本身"真实"。
...
**本质说明（θ 与自由现金流折现的关系）**：θ ≈ 长期 FCF / 净利润 的代理——`V×θ` 是"利润折现"与"自由现金流（FCF）折现"之间的桥：当各期转化率 θₜ 稳定时，

$$
V = \sum_{t} \frac{NP_{adj,t} \cdot \theta}{(1+R)^t} \;\approx\; \sum_{t} \frac{FCF_t}{(1+R)^t}
$$

即 FCF DCF 的**简化近似**（不是独立的新理论）。0.15/0.85 权重把重心压在成熟期转化率 θₓ 上（终值占估值主体），缓解了"常数 θ"在成长期（ΔWC 大）与成熟期转化率不同的近似误差。**以柏楚为例**：净现比约 1.0、θ₀ = 0.95，两法差异 <5%——近似误差可控。
...
**说明（净现金/净资产不参与估值）**：$V_0^*$ 之后**不单独加回净现金、不减净负债**——账上现金已被 θ 的"利润兑现"口径隐含处理，且现金可能用于发展或管理层不分配，不构成股东价值（见"一、设计哲学"）。若需评估"全部现金可分派"的上限，可在研报中作为敏感性披露（"若按全部可分配计，估值 +X%"），但**不进入基准估值**。
